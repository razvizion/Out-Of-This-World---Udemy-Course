...
- (void)viewDidLoad
{
    ...
    for(NSDictionary *dictionary in myPlanetsAsPropertyLists){
        OWSpaceObject *spaceObject = [self spacjeObjectForDictionary:dictionary];
...
//    NSLog(@"%@",myNumber);
//    NSNumber *floatNumber = [NSNumber numberWithFloat:3.33];
//    NSLog(@"%@",floatNumber);
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if([segue.destinationViewController isKindOfClass:[OWAddSpaceObjectViewController class]]){
        OWAddSpaceObjectViewController *addSpacjeObjectVC =segue.destinationViewController;
        addSpacjeObjectVC.delegate = self;
    }
}
...
-(void)addSpaceObject:(OWSpaceObject *)spaceObject
{
    ...
    if(!spaceObjectsAsPropertyList){
       spaceObjectsAsPropertyList = [[NSMutableArray alloc]init];
    }
...
-(NSDictionary *)spaceObjectAsAPropertyList:(OWSpaceObject *)spaceObject{
    ...
    NSDictionary *dictionaty = @{PLANET_NAME : spaceObject.name, PLANET_GRAVITY : @(spaceObject.gravitationalForce), PLANET_DIAMETER : @(spaceObject.diameter), PLANET_YEAR_LENGTH : @(spaceObject.yearLength), PLANET_DAY_LENGTH : @(spaceObject.dayLenghtl), PLANET_TEMPERATURE : @(spaceObject.temperature), PLANET_NUMBER_OF_MOONS : @(spaceObject.numberOfMoons), PLANET_NICKNAME : spaceObject.nickname, PLANET_INTERESTING_FACT : spaceObject.interestFact, PLANET_IMAGE: imageData};
    return dictionaty;
}

-(OWSpaceObject *)spacjeObjectForDictionary:(NSDictionary *)dictionary{
